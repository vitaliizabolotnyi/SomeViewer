...
public class Window : GameWindow
{
    ...
    protected override void OnLoad()
    {
        ...
        _volume = TryLoadVolume(DicomFolderCT);
        //_volume = TryLoadVolume(DicomFolderMR);

        _scenes = new ScenesController(BuildScenes(_volume));
    ...
    private IReadOnlyList<Scene> BuildScenes(VolumeData? volume)
    {
        if (volume == null)
        {
            return new[]
        ...
        return new[]
        {
            new Scene("Raycast DVR", () => new RaycastVolumeRenderer(_ctx, volume)),
            new Scene("Middle Slice", () => new SliceVolumeRenderer(_ctx, volume)),
            new Scene("Gradient", () => new GradientRenderer(_ctx)),
        };
    }
    ...
    private static VolumeData? TryLoadVolume(string dicomDir)
    {
        ...
        {
            Console.WriteLine($"DICOM folder '{dicomDir}' not found.");
            return null;
        }
        ...
        {
            Console.WriteLine($"Failed to load DICOM volume: {ex.Message}");
            return null;
        }
    }
